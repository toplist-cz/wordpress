...
class TopList_CZ_Widget extends WP_Widget {
	...
	function __construct() {
		...
		'description' => __('Integrates TOPlist statistics into your blog', 'toplist') );
	...
	function form($instance) {
		...
		echo '<p style="margin: 5px 10px;"><em>'.str_replace('%server%', $toplist_server, __('Your ID on <a href="https://www.%server%" target="_blank">www.%server%</a> server. If you don\'t have one yet, please <a href="https://www.%server%/register/" target="_blank">register</a>.', 'toplist')).'</em></p><hr />';
		...
		echo '<td>&nbsp;</td>';
		echo '<td><input id="' . $this->get_field_id('logo') . '" name="' . $this->get_field_name('logo') . '" type="radio" value="blank"'.($toplist_logo=='blank'?' checked':'').' /></td><td style="text-align: center">'.__('nothing', 'toplist').'</td>';
		echo '<td>&nbsp;</td>';
		...
		echo '<option value="default"' . ($toplist_display=='default'?' selected':'') . '>' . __('Default (specified by css of your theme)', 'toplist') . '</option>';
		echo '<option value="center"' . ($toplist_display=='center'?' selected':'') . '>' . __('Center (enforced)', 'toplist') . '</option>';
		echo '<option value="hidden"' . ($toplist_display=='hidden'?' selected':'') . '>' . __('Hidden (enforced)', 'toplist') . '</option>';
		echo '</select>';
		echo '</p><hr />';
		...
		_e('Monitor where visitors came from', 'toplist');
		echo '</label><br />';
		...
		_e('Monitor browser graphical resolution', 'toplist');
		echo '</label><br />';
		...
		_e('Monitor color depth', 'toplist');
		echo '</label><br />';
		...
		_e('Record webpage title', 'toplist');
		echo '</label><br />';
		echo '<label for="' . $this->get_field_name('start') . '">' . __('Start:', 'toplist') . '</label> ';
		echo '<input id="' . $this->get_field_id('start') . '" name="' . $this->get_field_name('start') . '" type="number" size="6" min="0" value="' . $toplist_start . '" />';
		echo '[<span title="'. __('Initial counter value', 'toplist').'" style="font-size:x-small; cursor:help;">' . __('what is this?', 'toplist') . '</span>]<br />';
		echo '<label for="' . $this->get_field_name('seccode') . '">' . __('Security code:', 'toplist') . '</label> ';
		echo '<input id="' . $this->get_field_id('seccode') . '" name="' . $this->get_field_name('seccode') . '" type="number" min="0" max="200" value="' . $toplist_seccode . '" />';
		echo '[<a href="https://o.toplist.cz/dokumentace/tipy-a-triky/#bezpecnostni-kod" target="_blank"><span style="font-size:x-small">' . __('what is this?', 'toplist') . '</span></a>]';
		echo '</p><hr />';
		...
		echo '</tr><tr>';
		echo '<td></td>';
		echo '<td><input id="' . $this->get_field_id('link') . '" name="' . $this->get_field_name('link') . '" type="radio" type="radio" value="stats"'.($toplist_link=='stats'?' checked':'').'>'.__('Detailed statistics', 'toplist').'</input></td>';
		echo '</tr></table>';
		echo '<hr />';

		// Statistics RESP API password
		echo '<p><label for="' . $this->get_field_name('serverkey') . '">'.__('Statistics password', 'toplist').': </label><input id="' . $this->get_field_id('serverkey') . '" name="' . $this->get_field_name('serverkey') . '" type="text" value="'.$toplist_serverkey.'" size="7" /></p>'."\n";
		echo '<p style="margin: 5px 10px;"><em>'.str_replace('%server%', $toplist_server, __('To show statistics right in Wordpress dashboard enter <a href="https://profi.%server%/serverKey" target="_blank">one time password</a>.', 'toplist')).'</em></p><hr />';

		// tracking admin users
		echo '<table><tr><td width="190px"><label for="' . $this->get_field_name('admindsbl') . '">';
		_e('WordPress admin logging', 'toplist');
		echo ': </label></td>';
		echo '<td>';
		...
		echo ">" . __('Enabled', 'toplist') . "</option>\n";

		echo "<option value='1'";
		if($toplist_admindsbl == '1')
			echo" selected='selected'";
		echo ">" . __('Disabled', 'toplist') . "</option>\n";
		...
		?>
		<p style="margin: 5px 10px;"><em><?php printf(__('Disabling this option will prevent visits from all logged-in WordPress administrators from showing up on your %1$s reports. Eventually, all logged-in users with role %2$s or higher are prevented from being logged by %1$s. Your role is %3$s.', 'toplist'), str_replace('toplist', 'TOPlist', $toplist_server), $roles_combo, _x(ucfirst(wp_get_current_user()->roles[0]), "User role")); ?></em></p>
		<?php
		echo '</td></tr></table>';
	}
	...
	function add_dashboard_widget() {
		...
		if (!$config)  // no config found => no dashboard widget
			return;
		if ($config['title'] == sprintf(__(self::_not_found_string, 'toplist'), $config['id']))
			return;
...
//    $fields .= "&weekday=$day";  // -- dělá problém na profi statistikách
	...
	private function get_toplist_stats_html($day = FALSE) {
		...
		if (strpos($body, '<html>Nespr') !== false)
			return new WP_Error('wrong_toplist_password', __( "Wrong or missing password to TOPlist.cz account", 'toplist'));
	...
	private function password_form() {
		$config = self::config();
		$msg = __('For displaying statistics, you must enter password to your TOPlist.cz account.', 'toplist');
		if ($config['server'] == 'toplist.sk')
			$msg = preg_replace('/(toplist)\.cz/i', '\1.sk', $msg);
		$id_label = __('ID', 'toplist');
		$pw_label = __('Password', 'toplist');
		$button = __('Save', 'toplist');
